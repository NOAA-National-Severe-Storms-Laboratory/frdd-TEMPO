...
contains
    ...
    subroutine thompson_init(l_mp_tables, hail_aware_flag, aerosol_aware_flag)
        ...
        if (l_mp_tables) then
            ...
            write(*,*) '--- thompson_init() called with hail_aware_flag = ' // trim(message)

            if (present(aerosol_aware_flag)) then
                configs%aerosol_aware = aerosol_aware_flag
                write(message, '(L1)') configs%aerosol_aware
                write(*,*) '--- thompson_init() called with aerosol_aware_flag = ' // trim(message)
            endif
        endif
            ...
            if (qr_acr_qg_exists) then ! Check again that file exists
                ...
                if (qr_acr_qg_filesize == qr_acr_qg_dim1size) then
                    ...
                    if (hail_aware_flag) then
                        write(*,*) '--- thompson_init() Cannot use hail-aware microphysics ' // &
        ...
        if (micro_init) then
            ...
            if (istat /= open_OK) then
                write(*,*) '--- thompson_init() failure opening CCN_ACTIVATE.BIN'
            endif
            ...
            if (istat /= open_OK) then
                write(*,*) '--- thompson_init() failure opening MP_THOMPSON_QRacrQG.DBL'
            endif
            ...
            open(unit=mp_unit,file='MP_THOMPSON_QRacrQS_DATA.DBL',form='unformatted',status='old',action='read', &
                iostat=istat,                  convert='big_endian')
            if (istat /= open_OK) then
                write(*,*) '--- thompson_init() failure opening MP_THOMPSON_QRacrQS.DBL'
            endif
            ...
            open(unit=mp_unit,file='MP_THOMPSON_freezeH2O_DATA.DBL',form='unformatted',status='old',action='read', &
                iostat=istat,                  convert='big_endian')
            if (istat /= open_OK) then
                write(*,*) '--- thompson_init() failure opening MP_THOMPSON_freezeH2O.DBL'
            endif
            ...
            open(unit=mp_unit,file='MP_THOMPSON_QIautQS_DATA.DBL',form='unformatted',status='old',action='read', &
                iostat=istat,                  convert='big_endian')
            if (istat /= open_OK) then
                write(*,*) '--- thompson_init() failure opening MP_THOMPSON_QIautQS.DBL'
            endif
    ...
    subroutine thompson_3d_to_1d_driver(qv, qc, qr, qi, qs, qg, qb, ni, nr, nc, ng, &
        ...
        j_loop:  do j = j_start, j_end
            i_loop:  do i = i_start, i_end
                ...
                if (itimestep == 1) then
                   write(*,*) '--- thompson_3d_to_1d_driver() configuration...'
                   write(message, '(L1)') configs%hail_aware
                   write(*,*) '       hail_aware_flag = ' // trim(message)
                   write(message, '(L1)') configs%aerosol_aware
                   write(*,*) '       aerosol_aware_flag = ' // trim(message)
                   write(*,*) 'calling mp_thompson_main() at itimestep = 1'
                endif
